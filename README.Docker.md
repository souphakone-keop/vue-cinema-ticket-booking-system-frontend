...
| `compose.yaml` | `frontend` service definition. Runs standalone as-is (`context: .`, no `depends_on`) for local testing; gets `depends_on: [backend]` added back when merged into the full system's compose file |

## Option A — `docker compose` (recommended for standalone use)

```bash
docker compose up --build   # build + start
docker compose up            # start with existing image (no rebuild)
docker compose down           # stop and remove
```

Opens on `http://localhost:3000`. `VITE_API_BASE_URL` defaults to
`http://localhost:8080` in `compose.yaml` — edit the `args:` block there if
your backend runs elsewhere, then re-run with `--build`.

## Option B — plain `docker build` / `docker run`

Useful when you want a one-off image without touching `compose.yaml`, or a
different `VITE_API_BASE_URL` without editing the file.
...
Port `80` inside the container is fixed (nginx); only the host-side port
(`3000` here) is arbitrary — just don't map it to `8080`, since the
backend already listens there.
...
(or repeat the `docker build` + `docker run` cycle from Option B).
...
## Running with the full system (backend + redis + ...)

`compose.yaml` here is standalone-friendly, but to run everything with one
command the backend team merges the `frontend` service into their main
`docker-compose.yml`, restoring `depends_on` and pointing `context` at
this repo's actual location relative to that file:
